...
import { getMovieByIdAction, removeCastAction, updateSeriesAction } from '../../../Redux/Actions/moviesActions';
...
const EditMovieSeries = () => {
    ...
    const { isLoading, isError, movie: series } = useSelector(
        (state) => state.getMovieById
    );
    const { isLoading: editLoading, isError: editError, isSuccess } = useSelector(
        (state) => state.updateSeries
    );
    ...
    const onSubmit = (data) => {
        ...
        dispatch(updateSeriesAction(id, {
            ...data,
            image: imageWithoutTitle,
            titleImage: imageTitle,
            casts: casts.length > 0 ? casts : series?.casts,
            typeFilm: "series",
        }));
    };
    ...
    useEffect(() => {
        if (series?._id !== id) {
            ...
            setValue("nameVn", series?.nameVn);
            setValue("name", series?.name);
            setValue("language", series?.language);
            setValue("year", series?.year);
            setValue("time", series?.time);
            setValue("category", series?.category);
            setValue("desc", series?.desc);
            setValue("imdbRating", series?.imdbRating);
            setValue("filmParts", series?.filmParts || []);

            setImageWithoutTitle(series?.image);
            setImageTitle(series?.titleImage);
        }
        ...
        if (isSuccess) {
            dispatch({ type: "UPDATE_SERIES_RESET" });
    ...
    }, [dispatch, id, series, setValue, isSuccess, editError, navigate]);
            ...
            {isLoading ? (
                ...
                <div className={sameClass}>
                    ...
                    <p className='text-border text-sm'>
                        Đã có lỗi xảy ra khi tải thông tin phim
                    </p>
                </div>
            ) : (
                <div className="flex flex-col gap-6">
                    <h2 className="text-xl font-bold">Chỉnh sửa phim bộ: "{series?.nameVn}"</h2>
                        ...
                        <div className="grid 2xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-4 grid-cols-2 gap-4">
                            {
                                ...
                                (casts?.length > 0 ? casts : series?.casts || []).map((user) => (
                                    <div
                                        key={user?.id}
